...
import {
  ...
  ArrowRight, ExternalLink, Save, Pencil,
} from 'lucide-react';
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center px-4 bg-black/80 backdrop-blur-sm">
      ...
      >
        ...
        {error && (
          <div className="mb-4 p-3 bg-red-500/10 border border-red-500/20 rounded-xl text-red-400 font-sans text-sm flex gap-2">
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          <div className="space-y-2">
            <label className="text-white/40 font-sans text-xs uppercase tracking-widest pl-1">Project Title</label>
            <input
              type="text"
              required
              value={title}
          ...
          <div className="space-y-2">
            <label className="text-white/40 font-sans text-xs uppercase tracking-widest pl-1">Brief / Description</label>
            <textarea
              value={description}
...
<head>
  <meta charset="UTF-8"/>
  <title>RevLabs Invoice #${payment.id.slice(0, 8).toUpperCase()}</title>
  <style>
    * { margin:0; padding:0; box-sizing:border-box; }
    body { font-family: 'Helvetica Neue', Helvetica, Arial, sans-serif; background:#000; color:#fff; padding:60px; max-width:700px; margin:0 auto; }
    .logo { font-size:32px; font-weight:300; letter-spacing:8px; text-transform:uppercase; margin-bottom:60px; }
    .logo span { font-style:italic; }
    .row { display:flex; justify-content:space-between; margin-bottom:8px; font-size:13px; }
    .label { color:#666; text-transform:uppercase; letter-spacing:2px; font-size:10px; }
    .divider { border:none; border-top:1px solid #222; margin:40px 0; }
    .status { display:inline-block; padding:4px 14px; border:1px solid #2a7a2a; color:#4ade80; font-size:11px; letter-spacing:2px; text-transform:uppercase; border-radius:20px; }
    h2 { font-size:11px; letter-spacing:4px; text-transform:uppercase; color:#444; margin-bottom:20px; }
    .amount { font-size:48px; font-weight:300; letter-spacing:-1px; }
    .footer { margin-top:80px; font-size:11px; color:#333; letter-spacing:1px; }
  </style>
</head>
<body>
  ...
  a.href = url;
  a.download = `revlabs-invoice-${payment.id.slice(0, 8)}.html`;
  a.click();
  ...
  return (
    <div className="bg-white/[0.02] border border-white/5 rounded-2xl p-8 max-w-lg">
      ...
      <div className="w-16 h-16 rounded-full bg-white/5 border border-white/10 flex items-center justify-center mb-8">
        <span className="text-2xl font-serif italic text-white/70">
          {(name || user?.email)?.[0]?.toUpperCase() || '?'}
        </span>
      </div>

      {saved && (
        <div className="mb-6 p-3 bg-green-500/10 border border-green-500/20 rounded-xl text-green-400 font-sans text-sm flex gap-2">
          <CheckCircle className="w-4 h-4 shrink-0 mt-0.5" /> Profile updated successfully.
        </div>
      )}
      ...
      <form onSubmit={handleSave} className="space-y-5">
        <div className="space-y-2">
          ...
          {editing ? (
            <input
              ...
              className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white font-sans focus:outline-none focus:border-white/30 transition-colors"
            />
          ) : (
        ...
        <div className="flex gap-3 pt-4 border-t border-white/5">
          {editing ? (
            <>
              <button type="submit" disabled={saving} className="flex items-center gap-2 px-6 py-2.5 bg-white text-black rounded-xl font-sans font-semibold text-sm hover:bg-white/90 transition-colors disabled:opacity-50">
                {saving ? <Loader2 className="w-4 h-4 animate-spin" /> : <Save className="w-4 h-4" />}
                {saving ? 'Saving...' : 'Save Changes'}
              </button>
              <button type="button" onClick={() => setEditing(false)} className="px-6 py-2.5 border border-white/10 text-white/50 rounded-xl font-sans text-sm hover:bg-white/5 transition-colors">
                Cancel
              </button>
            </>
          ) : (
            <button type="button" onClick={() => setEditing(true)} className="flex items-center gap-2 px-6 py-2.5 border border-white/10 text-white/60 rounded-xl font-sans text-sm hover:bg-white/5 hover:text-white transition-colors">
...
export default function Dashboard() {
  ...
  return (
    <div className="min-h-screen bg-[#000000] text-white pt-28 pb-20 px-6">
      <AnimatePresence>
        {showModal && (
          <NewReportModal onClose={() => setShowModal(false)} onCreate={createReport} />
        )}
      </AnimatePresence>
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-12 gap-6">
          <div>
            <p className="text-white/30 font-sans text-xs uppercase tracking-widest mb-2">Dashboard</p>
          ...
          <button
            onClick={logout}
            className="flex items-center gap-2 px-4 py-2 rounded-xl border border-white/10 text-white/40 font-sans text-sm hover:text-white hover:border-white/20 transition-colors w-fit"
          >
        ...
        <div className="flex gap-1 mb-8 border-b border-white/5 pb-px">
          {tabs.map(({ id, label, icon: Icon, count }) => (
            ...
            >
              ...
              {label}
              {count !== undefined && (
                <span className="text-xs bg-white/8 px-1.5 py-0.5 rounded-full">{count}</span>
              )}
            </button>
          ))}
        </div>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} key="reports">
            ...
            {rLoading ? (
              ...
              >
                ...
                <p className="text-white/30 font-sans max-w-xs mx-auto mb-8 leading-relaxed">
                  Submit a project brief and our creative team will take it from there.
                </p>
                <div className="flex flex-col sm:flex-row gap-3 justify-center">
                  <button
                    onClick={() => setShowModal(true)}
                    className="px-8 py-3 bg-white text-black rounded-full font-sans font-semibold hover:bg-white/90 transition-colors flex items-center gap-2 mx-auto sm:mx-0"
                  >
                    <Plus className="w-4 h-4" /> Submit a Brief
                  </button>
                  <Link
                    to="/services"
                    className="px-8 py-3 border border-white/10 text-white/60 rounded-full font-sans hover:bg-white/5 transition-colors flex items-center gap-2 mx-auto sm:mx-0"
                  >
                  ...
                  <motion.div key={report.id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}>
                    ...
                    >
                      <div className="space-y-2">
                        ...
                        <h3 className="text-lg font-sans font-medium text-white group-hover:text-white/90 transition-colors">
                          {report.title}
                        </h3>
                        {report.description && (
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} key="payments">
            ...
            {pLoading ? (
              ...
              >
                ...
                <p className="text-white/30 font-sans max-w-xs mx-auto mb-8 leading-relaxed">
                  Choose a service plan and complete a payment to see your history here.
                </p>
                <Link
                  to="/services"
                  className="px-8 py-3 bg-white text-black rounded-full font-sans font-semibold hover:bg-white/90 transition-colors inline-flex items-center gap-2"
                >
                  View Service Plans <ArrowRight className="w-4 h-4" />
                </Link>
              </motion.div>
            ) : (
              <div className="space-y-3">
                {payments.map((payment) => (
                  ...
                  >
                    <div className="space-y-2">
                      ...
                      <h3 className="text-lg font-sans font-medium text-white">
